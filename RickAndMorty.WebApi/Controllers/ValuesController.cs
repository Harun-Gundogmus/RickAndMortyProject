    ...
    ApplicationDbContext context) : ControllerBase
{
    ...
    {
        var response = await httpClient.GetAsync("https://rickandmorty.api.com/api/episode");

        EpisodeDto? episodeDto = await response.Content.ReadFromJsonAsync<EpisodeDto>();

        while(episodeDto!.Info.Next is null)
        {
            basadon:
            ...
            foreach (var item in episodeDto!.Results)
            {
                foreach (var characterUrl in item.Characters)
                {
                    ...
                    if (characterDto is not null)
                    {
                        ...
                        Origin? origin = context.Set<Origin>().FirstOrDefault(p => p.Url == characterDto.Location.Url && p.Name == characterDto.Location.Name);
